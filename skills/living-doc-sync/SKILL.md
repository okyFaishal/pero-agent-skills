...
3. **Perubahan Kontrak API / DTO / Model Domain**: Jika ada modifikasi schema payload, endpoint, atau model entitas → Wajib perbarui `docs/SystemSpec.md` (atau direktori pod `docs/system-spec/`) & `docs/decisions/SDR-[YYYYMMDDHHmm].md`.
...
7. **Perubahan Backlog Tugas**: Jika tugas selesai, dipecah, atau dependensi berubah → Wajib perbarui `docs/TaskBacklog.md` (atau direktori pod `docs/task-backlog/`) & `docs/decisions/TDR-[YYYYMMDDHHmm].md`.
...
| **Lapis 3: System Specification** | `docs/SystemSpec.md` atau `docs/system-spec/` / `SDR` | Skenario uji Gherkin, model entitas domain (ERD), kontrak API/event. |
...
- [ ] Perubahan endpoint, schema, atau method signature telah tercatat di `docs/SystemSpec.md` (atau `docs/system-spec/index.md`).
